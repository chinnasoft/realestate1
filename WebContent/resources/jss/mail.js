$(document).ready(
		function() {
			...
			$('#emailtable tbody').on('click','tr',function () {
			 	$("input#mailserver").val($(this).find('td:first').next().text());
  				$("input#portnumber").val($(this).find('td:first').next().next().text());
				$("input#frommail").val($(this).find('td:first').next().next().next().text());
				$("select#pt").val($(this).find('td:first').next().next().next().next().text()).change();
				$("input#tomail").val($(this).find('td:first').next().next().next().next().next().text());
			}); 
			 ...
			 $(document).on("click", "#delMail", function(){                        
          	    ...
          	    var $row = $(this).closest("tr");                         
					    ...
					    $.get('DeleteConfMail', {
							...
							$.each(responseJson, function(key98,
									value98) {									
								});
						});

					    $("input#mailserver").val("");
						$("input#portnumber").val("");
						$("input#frommail").val("");							
			...
			$('#send').click(
					function(event) {
						var $r1 = $("input#qid").val();
						var $r2 = $("select#frommail").val();
						var $r3 = $("input#tomail").val();
						var $r4 = $("input#fileID").val();
						$('#myModal54').modal('show');
						
						$.get('SendQuotationMail', {
							qid : $r1,
							frommail : $r2,
							tomail : $r3,
							file : $r4
